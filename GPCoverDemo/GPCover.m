...
static GPCoverStyle     _style;           // 遮罩类型
static GPCoverShowStyle _showStyle;       // 显示类型
static GPCoverAnimStyle _animStyle;       // 动画类型
static BOOL             _hasCover;        // 遮罩是否已经显示的判断值
static BOOL             _isHideStatusBar; // 遮罩是否遮盖状态栏

// 分离动画类型
static GPCoverShowAnimStyle _showAnimStyle;
static GPCoverHideAnimStyle _hideAnimStyle;
...
+ (instancetype)transparentCoverWithTarget:(id)target action:(SEL)action
{
    ...
    bgView.size = CGSizeMake(KScreenW, KScreenH);
...
{
    if (_animated) {
        _contentView.y = KScreenH;
        [UIView animateWithDuration:0.25 animations:^{
            _contentView.y = KScreenH - _contentView.height;
...
// 常见遮罩
+ (void)topCover:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    GPCoverAnimStyle animStyle;
    if (animated) {
        animStyle = GPCoverAnimStyleTop;
    }else{
        animStyle = GPCoverAnimStyleNone;
    }
    
    [self coverFrom:fromView contentView:contentView style:style showStyle:GPCoverShowStyleTop animStyle:animStyle notClick:notClick];
}

+ (void)bottomCoverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    GPCoverAnimStyle animStyle;
    if (animated) {
        animStyle = GPCoverAnimStyleBottom;
    }else{
        animStyle = GPCoverAnimStyleNone;
    }
    
    [self coverFrom:fromView contentView:contentView style:style showStyle:GPCoverShowStyleBottom animStyle:animStyle notClick:notClick];
}

+ (void)centerCover:(UIView *)contentView style:(GPCoverStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    GPCoverAnimStyle animStyle;
    if (animated) {
        animStyle = GPCoverAnimStyleCenter;
    }else{
        animStyle = GPCoverAnimStyleNone;
    }
    
    [self coverFrom:[self getKeyWindow] contentView:contentView style:style showStyle:GPCoverShowStyleCenter animStyle:animStyle notClick:notClick];
}
 ...
 */
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style showStyle:(GPCoverShowStyle)showStyle animStyle:(GPCoverAnimStyle)animStyle notClick:(BOOL)notClick
{
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style showStyle:(GPCoverShowStyle)showStyle animStyle:(GPCoverAnimStyle)animStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock
{
    if ([self hasCover]) return;
    ...
    if (style == GPCoverStyleTranslucent) { // 半透明
        cover.backgroundColor = [UIColor blackColor];
        cover.alpha = kAlpha;
        [self addTap:cover];
    }else if (style == GPCoverStyleTransparent){  // 全透明
        cover.backgroundColor = [UIColor clearColor];
        [cover addSubview:[self gp_transparentBgView]];
    }else{ // 毛玻璃，高斯模糊
        cover.backgroundColor = [UIColor clearColor];
        [self addTap:cover];
        // 添加高斯模糊效果,添加毛玻璃效果
...
+ (void)showView
{
    ...
    if (_showStyle == GPCoverShowStyleTop) {
        if (_animStyle == GPCoverAnimStyleTop) {
    ...
    }else if (_showStyle == GPCoverShowStyleCenter){
        if (_animStyle == GPCoverAnimStyleTop) { // 上进下出
            _contentView.y = -_contentView.height;
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.center = _fromView.center;
            }completion:^(BOOL finished) {
        ...
        }else if (_animStyle == GPCoverAnimStyleCenter) { // 中间动画
            _contentView.center = _fromView.center;
            [self animationAlert:_contentView];
        }else if (_animStyle == GPCoverAnimStyleBottom) { // 下进上出
            _contentView.y = KScreenH;
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.center = _fromView.center;
            }completion:^(BOOL finished) {
    ...
    }else if (_showStyle == GPCoverShowStyleBottom){
        if (_animStyle == GPCoverAnimStyleBottom) {
            _contentView.y = KScreenH;
            [UIView animateWithDuration:0.25 animations:^{
                _contentView.y = KScreenH - _contentView.height;
            }completion:^(BOOL finished) {
...
+ (void)hideView
{
    ...
    if (_showStyle == GPCoverShowStyleTop) {
        if (_animStyle == GPCoverAnimStyleTop) { // 上进上出
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.y = -_contentView.height;
            }completion:^(BOOL finished) {
                [self remove];
        ...
        if (_animStyle == GPCoverAnimStyleBottom) {  // 下进下出
            [UIView animateWithDuration:0.25 animations:^{
                _contentView.y = KScreenH;
            }completion:^(BOOL finished) {
                [self remove];
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style showStyle:(GPCoverShowStyle)showStyle showAnimStyle:(GPCoverShowAnimStyle)showAnimStyle hideAnimStyle:(GPCoverHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick
{
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(GPCoverStyle)style showStyle:(GPCoverShowStyle)showStyle showAnimStyle:(GPCoverShowAnimStyle)showAnimStyle hideAnimStyle:(GPCoverHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock
{
    if ([self hasCover]) return;
    ...
    switch (style) {
        case GPCoverStyleTranslucent: // 半透明
            [self setupTranslucentCover:cover];
            break;
        case GPCoverStyleTransparent: // 全透明
            [self setupTransparentCover:cover];
            break;
        case GPCoverStyleBlur:        // 高斯模糊
...
+ (void)coverHideStatusBarWithContentView:(UIView *)contentView style:(GPCoverStyle)style showStyle:(GPCoverShowStyle)showStyle showAnimStyle:(GPCoverShowAnimStyle)showAnimStyle hideAnimStyle:(GPCoverHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock {
...
+ (void)showCover
{
    ...
    switch (_showStyle) {
        case GPCoverShowStyleTop:  // 显示在顶部
        {
            if (_showAnimStyle == GPCoverShowAnimStyleTop) {
                _contentView.y = -_contentView.height;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.y = 0;
                }completion:^(BOOL finished) {
            ...
            break;
        case GPCoverShowStyleCenter: // 显示在中间
        {
            if (_showAnimStyle == GPCoverShowAnimStyleTop) { // 上进
                _contentView.y = -_contentView.height;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.center = _fromView.center;
                }completion:^(BOOL finished) {
            ...
            }else if (_showAnimStyle == GPCoverShowAnimStyleCenter) { // 中间动画
                _contentView.center = _fromView.center;
                [self animationAlert:_contentView];
            }else if (_showAnimStyle == GPCoverShowAnimStyleBottom) { // 下进
                _contentView.y = KScreenH;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.center = _fromView.center;
                }completion:^(BOOL finished) {
            ...
            break;
        case GPCoverShowStyleBottom:  // 显示在底部
        {
            if (_showAnimStyle == GPCoverShowAnimStyleBottom) {
                _contentView.y = KScreenH;
                [UIView animateWithDuration:0.25 animations:^{
                    _contentView.y = KScreenH - _contentView.height;
                }completion:^(BOOL finished) {
...
+ (void)hideCover
{
    ...
    switch (_showStyle) {
        case GPCoverShowStyleTop:  // 显示在顶部
        {
            if (_hideAnimStyle == GPCoverHideAnimStyleTop) {
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.y = -_contentView.height;
                }completion:^(BOOL finished) {
                    [self remove];
        ...
        case GPCoverShowStyleCenter:  // 显示在中间
        {
            if (_hideAnimStyle == GPCoverHideAnimStyleTop) { // 上出
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.y = -_contentView.height;
                }completion:^(BOOL finished) {
                    [self remove];
                }];
            }else if (_hideAnimStyle == GPCoverHideAnimStyleCenter) { // 中间动画
                [self remove];
            }else if (_hideAnimStyle == GPCoverHideAnimStyleBottom) { // 下出
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.y = KScreenH;
                }completion:^(BOOL finished) {
                    [self remove];
        ...
        case GPCoverShowStyleBottom:  // 显示在底部
        {
            if (_hideAnimStyle == GPCoverHideAnimStyleBottom) {
                [UIView animateWithDuration:0.25 animations:^{
                    _contentView.y = KScreenH;
                }completion:^(BOOL finished) {
                    [self remove];
...
+ (void)layoutSubViews {
    ...
    switch (_showStyle) {
        case GPCoverShowStyleTop:
        {
            _contentView.y = 0;
        }
            break;
        case GPCoverShowStyleCenter:
        {
            _contentView.center = _fromView.center;
        }
            break;
        case GPCoverShowStyleBottom:
        {
            _contentView.y = KScreenH - _contentView.height;
        }
            break;
